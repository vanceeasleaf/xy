...
class xymodel (object):
	...
	def __init__(self,SIZE,T,J=1,h=0):
		...
		self.E = 0.0
		self.cos = dict([(i*5,(np.cos(i*5/180.0*3.1415926))) for i in range(0,72)])		 	
		self.be = dict([(i*5,np.exp(self.cos[i*5]/T)) for i in range(0,72)])
	...
	def MCstep(self, N=1):
		...
		for i in xrange(0,N):
			...
			self.change = 0		
			if i%1 == 0:
				output += "%f\t%f\t%f\n"%(  sum(sum(np.cos(self.m*3.14/180.0))) / float(self.SIZE * self.SIZE), 
											sum(sum(np.sin(self.m*3.14/180.0))) / float(self.SIZE * self.SIZE), 
	...
	def step(self, x, y):
		...
		p =  np.prod([self.be[i] for i in diff1]) / np.prod([self.be[i] for i in diff0])
