...
# @Last Modified time: 2017-06-19 21:58:15
...
class Ising(object):
    ...
    def E0(self, m):
        ...
        for x in range(0, SIZE):
            for y in range(0, SIZE):
                factor = y % 2 * 2 - 1
                links = m[x][(y + 1) % SIZE] + m[x][(y - 1) % SIZE] + m[(
                    x + 1) % SIZE][y] + m[(x - 1) % SIZE][y] + m[(
                        x - factor) % SIZE][(y - 1) % SIZE] + m[(
                            x - factor) % SIZE][(y + 1) % SIZE]
    ...
    def step(self, x, y):
        ...
        factor = y % 2 * 2 - 1
        links = self.m[x][(y + 1) % SIZE] + self.m[x][(
            y - 1) % SIZE] + self.m[(x + 1) % SIZE][y] + self.m[(
                x - 1) % SIZE][y] + self.m[(x - factor) % SIZE][(
                    y - 1) % SIZE] + self.m[(x - factor) % SIZE][(
                        y + 1) % SIZE]
    ...
    def MCstep(self, N):
        ...
        for i in range(0, N):
            ...
            if i % 1 == 0:
                output += "%f\t%f\n" % (
                    sum(sum(self.m)) / float(self.SIZE * self.SIZE),
