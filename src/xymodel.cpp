...
* @Last Modified time: 2017-06-19 22:14:01
*/
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
#define MAXL 129;
double *e1;
void calculate(int l, int q, double t, double *sumh, double *sumh2,
...
void xymodel(int l, int q) {
  double t = .01, tstart = .1, tend = 1.50, deltat = .01, sumh = 0, sumh2 = 0,
         c = 0, summ = 0, summ2 = 0, x = 0;
  ...
  fprintf(
      fp,
      "temperature\tenergy\theat_capacity\tmagnetization\tsusceptibility\n");
  e1 = new double[END];
...
int isMK(int x, int y) { return (x == 0 && y == 0) ? 1 : 0; }

double e_spin(int s1, int s2) { return -coss[abs(s1 - s2)]; }
